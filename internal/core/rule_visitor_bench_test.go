...
func BenchmarkConditionVisitNoFact(b *testing.B) {
	...
	aruleV := NewArishemRuleVisitor(nil)
	dc, _ := NewArishemDataCtx(context.Background(), "{}", &testFeatureFetcher{})
	for i := 0; i < b.N; i++ {
...
func BenchmarkConditionVisitFactVar(b *testing.B) {
	...
	cdtCtx, _ := parser.ParseArishemCondition(cdtExpr)
	aruleV := NewArishemRuleVisitor(nil)
	dc, _ := NewArishemDataCtx(context.Background(), `{"number1":100,"numbers":["10",1,99.9]}`, &testFeatureFetcher{})
	for i := 0; i < b.N; i++ {
		aruleV.VisitCondition(cdtCtx, dc, nil)
	}
}
